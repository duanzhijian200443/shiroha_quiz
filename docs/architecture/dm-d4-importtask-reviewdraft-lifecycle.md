...
| Retry identity and stale attempt isolation | `ImportTaskCoordinator.retryOcrTask()` and `TaskManager.restartAttempt()` preserve task ID, require the next attempt number, and accept a fresh token/trace; coordinator checks current attempt before applying results. | Aligned; D4-D2 must make durable races equally strict. |
...
| ReviewDraft storage and CAS | `TaskManager.saveReviewDraft()` stores `parsedData` plus `_reviewDraftRevision`; `_saveReviewDraftNow()` rejects an expected-revision mismatch without mutating the newer draft. | Aligned; keep the CAS durable and typed. |
...
| Active/review cleanup guard | `TaskCenterProjection` hides cancellation/delete actions for active states, but current persistence methods are broader than the presentation guard. | Follow-up gap: D4-D1 must enforce the guard at the Application/persistence authority. |
