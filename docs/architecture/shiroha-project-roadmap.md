...
| **IMPORT-RD0** | COMPLETE | A successful `documentImportEntry` parse persists `pendingReview` and materializes durable ReviewDraft revision 1 through the existing CAS writer (`expectedRevision = 0`) without adding a writer or bypassing the commit lease; later writes continue normal CAS from the loaded revision. The auto-commit path holds to that RD0 revision and refuses to rewrite an existing bank's folder mapping or to append into a concurrently created proposed bank. Contract: `docs/architecture/import-rd0-initial-review-draft.md`. |
| **OCR-RGN-1** | IN REVIEW (PR #179, not merged) | The OCR regionizer accepts the real 2020/2021 数学一 layouts: range section headings including a provider LaTeX rendering (`$17\sim 22$小题`), the restricted single-sided `N）` marker, a heading's kind scoped to the question numbers it declared, and a bounded recovery when the provider drops a marker's leading digit. A `failed_no_question_regions` attempt keeps a whitelisted, text-free `ocrRegionizer` diagnostic block. Maintainer live acceptance passed on both files (2020: 23 questions, assembled types 0/2/3, one repair; 2021: 22 questions, three sections, zero sequence rejections). Independent Verifier/Reviewer and merge remain pending. |
