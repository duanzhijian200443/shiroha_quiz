  ...
  void _startExam() async {
    if (_singleCount == 0 && _subjectiveCount == 0) {
      ScaffoldMessenger.of(context)
          .showSnackBar(const SnackBar(content: Text('题目数量不能全为 0')));
      return;
    }
    ...
    try {
      // 1. 极速抽题
      final questions = await ExamRepository.instance.generateMockExamPaper(
          widget.initialBank, _singleCount, _subjectiveCount);
      if (!mounted) return;

      if (questions.isEmpty) {
        Navigator.pop(context);
        ScaffoldMessenger.of(context)
            .showSnackBar(const SnackBar(content: Text('当前题库题目不足，无法生成试卷')));
        return;
      }
      ...
      Navigator.pop(context);
      ScaffoldMessenger.of(context)
          .showSnackBar(SnackBar(content: Text('生成试卷失败: $e')));
    }
  }
          ...
          children: [
            Container(
              ...
              child: Column(
                ...
                children: [
                  ...
                  const Text(
                      '1. 考试期间将强制保持屏幕常亮。\n2. 考试中途退出将丢失所有作答进度。\n3. 提交后客观题自动批改，主观题交由 AI 阅卷。',
                      style: TextStyle(
                          color: Colors.redAccent, height: 1.5, fontSize: 13)),
                ],
              ),
            ),
            const SizedBox(height: 32),
            Text('目标题库：${widget.initialBank}',
            ...
            Card(
              ...
              child: Padding(
                ...
                child: Column(
                  children: [
                    _buildCounterRow('单选题数量', _singleCount,
                        (v) => setState(() => _singleCount = v), 0, 100, '题'),
                    const Divider(height: 1),
                    _buildCounterRow(
                        '主观题数量',
                        _subjectiveCount,
                        (v) => setState(() => _subjectiveCount = v),
                        0,
                        20,
                        '题'),
                    const Divider(height: 1),
