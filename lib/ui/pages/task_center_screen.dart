...
class TaskCenterScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('传输与解析中心', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 18)),
        elevation: 0,
        actions: [
          TextButton.icon(
            icon: const Icon(Icons.clear_all_rounded, size: 18),
            label: const Text('清除已完成'),
            onPressed: () => TaskManager.instance.clearCompleted(),
        ...
        builder: (context, _) {
            ...
            return Center(
              child: Column(
                ...
                children: [
                  ...
                  const Text('当前没有后台任务', style: TextStyle(color: Colors.grey, fontSize: 16, fontWeight: FontWeight.bold)),
                  const SizedBox(height: 8),
                  const Text('所有的解析和下载记录将显示在这里', style: TextStyle(color: Colors.grey, fontSize: 13)),
                ],
              ),
            );
          }
          ...
          return ListView.builder(
            ...
            itemBuilder: (context, index) {
              ...
              if (t.status == TaskStatus.completed) { icon = Icons.check_circle; iconColor = Colors.green; } 
              else if (t.status == TaskStatus.error) { icon = Icons.error_rounded; iconColor = Colors.redAccent; }
              else if (t.status == TaskStatus.pendingReview) { icon = Icons.rule_rounded; iconColor = Colors.orange; } // 待校对图标
              ...
              return Card(
                ...
                child: InkWell(
                  ...
                  onTap: t.status == TaskStatus.pendingReview && t.parsedData != null
                      ? () {
                          // 点击待校对任务，弹出暂存区
                          Navigator.push(
                            context,
                      ...
                      children: [
                        ...
                        if (t.status == TaskStatus.pendingReview)
                          const Padding(
                            ...
                            child: Icon(Icons.arrow_forward_ios_rounded, size: 14, color: Colors.orange),
                          )
                      ],
                    ),
                  ),
