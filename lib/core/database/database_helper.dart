...
final class QuestionDeletePersistenceException implements Exception {
  const QuestionDeletePersistenceException(this.failure);

  final QuestionDeletePersistenceFailure failure;
      ...
      await db.transaction((txn) async {
        try {
          ...
          if (examReferenceTable.isNotEmpty) {
            ...
            if (references.isNotEmpty) {
              throw const QuestionDeletePersistenceException(
        ...
        final ftsTable = await txn.rawQuery(
          "SELECT type FROM sqlite_master WHERE name = 'questions_fts' "
          'LIMIT 1',
        );
