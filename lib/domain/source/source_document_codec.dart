...
final class SourceDocumentCodec {
  ...
  Map<String, Object?> encode(SourceDocument document) {
    return _encodeDocument(document, schemaVersion);
  }

  /// Losslessly re-encodes the legacy SourceDocument v1 envelope.
  ///
  /// Spanned normalized tables cannot be represented by the v1 rows-only
  /// shape and therefore fail closed instead of flattening their geometry.
  ...
  List<Object?> _encodeLegacyRows(List<List<RichContent>> rows) {
    return rows
        .map(
          (row) => row.map<Object?>(_richContentCodec.encode).toList(),
        )
        .toList();
  }
...
const _normalizedTablePartKeys = <String>{
  'type',
  'sourceRef',
  'structure',
};
