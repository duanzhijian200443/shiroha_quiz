...
void main() {
  group('catalog', () {
    test(
      ...
      expect(AgentWriteProposalToolCatalog.definition.name,
  ...
  group('strict payload validation', () {
    ...
    test('unknown, missing and duplicated keys fail as invalid_request',
        () async {
      final unknownKey = await dispatcher.dispatch(_call(jsonEncode(
        <String, Object?>{
          'target': _storageId,
          'answer': <String, Object?>{
      ...
      final bothKinds = await dispatcher.dispatch(_call(jsonEncode(
        <String, Object?>{
          'target': _storageId,
          'answer': <String, Object?>{
            'option_numbers': <int>[1],
    ...
    test('malformed types and out-of-bounds sizes fail safely', () async {
      final badTarget = await dispatcher.dispatch(_call(jsonEncode(
        <String, Object?>{
          'target': 42,
          'answer': <String, Object?>{
            'option_numbers': <int>[1]
      ...
      final hugeTarget = await dispatcher.dispatch(_call(jsonEncode(
        <String, Object?>{
          'target': 'x' * 129,
          'answer': <String, Object?>{
            'option_numbers': <int>[1]
      ...
      final badNumbers = await dispatcher.dispatch(_call(jsonEncode(
        <String, Object?>{
          'target': _storageId,
          'answer': <String, Object?>{
    ...
    test('unknown node types, unknown keys and oversized nodes fail safely',
        () async {
      final unknownType = await dispatcher.dispatch(_call(_contentArguments(
        <Map<String, Object?>>[
      ...
      final unknownKey = await dispatcher.dispatch(_call(_contentArguments(
        <Map<String, Object?>>[
      ...
      final missingField = await dispatcher.dispatch(_call(_contentArguments(
        <Map<String, Object?>>[
      ...
      final tooManyNodes = await dispatcher.dispatch(_call(_contentArguments(
    ...
    test('malformed JSON and non-object arguments fail as invalid_request',
        () async {
  ...
  group('admission safety and conversion', () {
    ...
    test('option numbers convert to option ids in number order', () async {
      ...
      final response = await dispatcher.dispatch(_call(_choiceArguments(
        <int>[3, 1],
      )));
    ...
    test(
      ...
      final response = await outOfRange.dispatch(_call(_choiceArguments(
        <int>[4],
      )));
  ...
  group('successful staging', () {
    ...
    test('stages a content proposal with an exact preview and no commit',
        () async {
      final response = await dispatcher.dispatch(_call(_contentArguments(
        <Map<String, Object?>>[
          <String, Object?>{'type': 'text', 'text': 'answer '},
    ...
    test('runtime source context is injected and never taken from arguments',
        () async {
      await dispatcher.dispatch(_call(_contentArguments(
        <Map<String, Object?>>[
          <String, Object?>{'type': 'text', 'text': 'answer'},
        ],
      )));
    ...
    test('semantic replay returns the same proposal id', () async {
      ...
      expect(
        (second['result'] as Map<String, Object?>)['outcome'],
        'pending',
      );
    });

    test('semantic replay after approval reports the committed outcome',
        () async {
      final arguments = _contentArguments(<Map<String, Object?>>[
        <String, Object?>{'type': 'text', 'text': 'answer'},
      ]);
