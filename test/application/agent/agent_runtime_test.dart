...
void main() {
    ...
    group('Tool budget graceful degradation', () {
      ...
      test('Case H: tool phase closure disables native web search', () async {
        ...
        final harness = _Harness(
          ...
          scripts: <_Script>[
            _toolRound(calls, const _TestContinuationState('s1')),
            _finalAnswer('Answer after web disabled'),
          ],
        );
        final (conversationId, userMessageId) = await harness.seedUser(
          'question',
        ...
        expect(harness.provider.requests[1].enableNativeWebSearch, isFalse);
        expect(harness.provider.requests[1].tools, isEmpty);
      });

      test(
          'Case I: diagnostics logs distinguish local_call_limit_exceeded vs tool_round_limit_exceeded',
          () async {
        final sink = _MemoryLogSink();
        AppLogger.setSink(sink);

        // Sub-case 1: local_call_limit_exceeded
        final localCalls = List<AgentProviderFunctionCall>.generate(
          9,
          (index) => _call('call-local-$index'),
        );
        final harnessLocal = _Harness(
          scripts: <_Script>[
            _toolRound(localCalls, const _TestContinuationState('s1')),
            _finalAnswer('Local call bounded answer'),
          ],
        );
        final (convId1, msgId1) = await harnessLocal.seedUser('q1');
        await harnessLocal.runtime
            .startTurn(conversationId: convId1, userMessageId: msgId1)
            .result;
        await AppLogger.flush();

        final localClosedEvent = sink.records.firstWhere(
          (r) =>
              r.data['stage'] == 'tool_phase_closed' &&
              r.data['reason'] == 'local_call_limit_exceeded',
        );
        expect(localClosedEvent.data['reason'], 'local_call_limit_exceeded');

        // Sub-case 2: tool_round_limit_exceeded
        const state = _TestContinuationState('s');
        final harnessRound = _Harness(
          scripts: <_Script>[
            _toolRound(<AgentProviderFunctionCall>[_call('call-r1')], state),
        ...
        final (convId2, msgId2) = await harnessRound.seedUser('q2');
        await harnessRound.runtime
            .startTurn(conversationId: convId2, userMessageId: msgId2)
            .result;
        await AppLogger.flush();

        final roundClosedEvent = sink.records.firstWhere(
          (r) =>
              r.data['stage'] == 'tool_phase_closed' &&
              r.data['reason'] == 'tool_round_limit_exceeded',
        );
        expect(roundClosedEvent.data['reason'], 'tool_round_limit_exceeded');
        expect(roundClosedEvent.data['toolRoundsUsed'], 4);
      });
    });
  ...
  group('W0 proposal integration', () {
    test(
      ...
      expect(unwiredPrompt, isNot(contains('no autonomous mutation')));
      expect(
        unwiredPrompt,
        isNot(contains('Never claim that writes occurred.')),
      );
      expect(
        unwiredPrompt,
      ...
      expect(wiredPrompt, isNot(contains('no autonomous mutation')));
      expect(wiredPrompt, isNot(contains('Never claim that writes occurred.')));
    });
  ...
  group('bounded fallback v0', () {
    test('Primary success: Fallback provider is never called', () async {
      final harness = _Harness(
    ...
    test(
        'Primary rateLimited before output -> fallback used exactly once -> success',
        () async {
      final harness = _Harness(
        fallbackProfileId: 'profile-fallback',
        scripts: <_Script>[
      ...
      expect(messages.first.role, ConversationMessageRole.user);
      expect(messages.last.role, ConversationMessageRole.assistant);
      expect(messages.last.content, 'Fallback Answer');
    });

    test('Primary temporarilyUnavailable before output -> fallback success',
        () async {
      final harness = _Harness(
        fallbackProfileId: 'profile-fallback',
        scripts: <_Script>[
    ...
    test('Fallback itself fails -> no third attempt -> final typed failure',
      ...
      expect(_failureOf(result), AgentTurnFailure.authentication);
      expect(harness.provider.callCount, 1);
      expect(harness.fallbackProvider.callCount, 1);
    });

    test('Primary emits TextDelta then fails -> fallback NOT called', () async {
      final harness = _Harness(
        fallbackProfileId: 'profile-fallback',
        scripts: <_Script>[
          (request, token) async* {
    ...
    test('Primary emits WebSearch progress then fails -> fallback NOT called',
        () async {
      final harness = _Harness(
        fallbackProfileId: 'profile-fallback',
        webEnabled: true,
    ...
    test(
        'Primary completes tool-call round -> local tool executes once -> subsequent failure -> fallback NOT called',
        () async {
      final harness = _Harness(
        fallbackProfileId: 'profile-fallback',
        scripts: <_Script>[
          _toolRound(
            <_Call>[_call('call-1', name: 'list_question_banks')],
    ...
    test(
        'W0 proposal is staged -> subsequent provider failure -> fallback NOT called -> proposal count exactly one',
        () async {
      final harness = _Harness(
        fallbackProfileId: 'profile-fallback',
        wireProposal: true,
        scripts: <_Script>[
          _toolRound(
            <_Call>[_proposalCall('call-prop-1')],
            const _TestContinuationState('round-1'),
          ),
          (request, token) async* {
            throw const AgentProviderException(
    ...
    test(
        'StudyPlanDraft is staged -> subsequent provider failure -> fallback NOT called -> draft count exactly one',
        () async {
      final harness = _Harness(
        fallbackProfileId: 'profile-fallback',
        wireStudyPlan: true,
        scripts: <_Script>[
          _toolRound(
            <_Call>[_studyPlanCall('call-plan-1')],
            const _TestContinuationState('round-1'),
          ),
          (request, token) async* {
            throw const AgentProviderException(
      ...
      expect(_failureOf(result), AgentTurnFailure.rateLimited);
      expect(events.whereType<AgentTurnStudyPlanDraftStaged>(), hasLength(1));
      expect(harness.fallbackProvider.callCount, 0);
    });

    test(
        'retrieval-approved turn + Primary eligible failure -> fallback NOT called',
        () async {
      final harness = _Harness(
        fallbackProfileId: 'profile-fallback',
        wireRetrieval: true,
    ...
    test('no approved file-content egress -> normal fallback still eligible',
      ...
      final harness = _Harness(
        ...
        fallbackScripts: <_Script>[
          _finalAnswer('Fallback Answer Retrieval Turn'),
        ],
      );
      final (conversationId, userMessageId) = await harness.seedUser(
        'question',
      ...
      expect(
        (result as AgentTurnSuccess).assistantMessage.content,
        'Fallback Answer Retrieval Turn',
      );
      expect(harness.provider.callCount, 1);
      expect(harness.fallbackProvider.callCount, 1);
    });

    test('user cancel before fallback transition -> fallback NOT started',
        () async {
      final primaryRunning = Completer<void>();
      final harness = _Harness(
        fallbackProfileId: 'profile-fallback',
        scripts: <_Script>[
          (request, token) async* {
            primaryRunning.complete();
            await token.whenCancelled;
            throw const AgentProviderException(AgentProviderFailure.cancelled);
          },
        ],
    ...
    test('global timeout -> fallback does not reset timeout/budget', () async {
      final harness = _Harness(
        fallbackProfileId: 'profile-fallback',
        limits: const AgentRuntimeLimits(
  ...
  group('OBS-1 agent trace', () {
    ...
    test(
        'tool-round overflow logs tool_phase_closed with reason and bounds execution',
        () async {
      final sink = _MemoryLogSink();
      AppLogger.setSink(sink);
      const state = _TestContinuationState('s');
      ...
      expect(result, isA<AgentTurnSuccess>());
      expect(harness.dispatcher.calls, hasLength(4));
      final closed = sink.records.firstWhere(
        (r) => r.data['stage'] == 'tool_phase_closed',
      );
      expect(closed.data['reason'], 'tool_round_limit_exceeded');
      expect(closed.data['toolRoundsUsed'], 4);
    });
    ...
    test('malformed tool identity never reaches the diagnostic summary',
      ...
      final harness = _Harness(scripts: <_Script>[
        ...
        _finalAnswer('Safe prose answer'),
      ]);
      final (conversationId, userMessageId) = await harness.seedUser(
        'question',
      );
      ...
      expect(result, isA<AgentTurnSuccess>());
      final toolRecords = sink.records
          .where((r) =>
              r.data['stage'] == 'tool_call_started' ||
              r.data['stage'] == 'tool_call_completed')
          .toList();
      expect(toolRecords, hasLength(8)); // 4 started + 4 completed
      for (final r in toolRecords) {
        expect(r.data['toolName'], 'unknown_tool');
        expect(r.data['callId'], 'invalid_call_id');
      }
      final encoded =
          sink.records.map((record) => record.toJson().toString()).join('\n');
      expect(encoded, isNot(contains(sentinel)));
...
final class _Harness {
  _Harness({
    ...
    List<_Script> fallbackScripts = const <_Script>[],
    AgentProviderCapabilities? capabilities,
    AgentProviderCapabilities? fallbackCapabilities,
    ...
    AgentReasoningEffort reasoningEffort = AgentReasoningEffort.high,
        ...
        profileResolver = _FakeProfileResolver(
          ...
          fallbackProfile: fallbackProfileId == null
              ? null
    ...
    fallbackProvider = _ScriptedProvider(
      fallbackScripts,
    ...
    runtime = ShirohaAgentRuntime(
      ...
      providerFactory: (resolved) {
        if (resolved.profile.profileId == fallbackProfileId) {
  ...
  late final _ScriptedProvider fallbackProvider;
...
final class _FakeProfileResolver implements AgentProviderProfileResolverPort {
  ...
  Future<AgentProviderProfile?> resolveMainProfile(String profileId) async {
    ...
    if (fallbackProfile?.profileId == profileId) return fallbackProfile;
    return null;
  }
}
