...
void main() {
  ...
  group('upgrade matrix', () {
    ...
    test('existing exact sidecar is accepted idempotently with data intact',
      ...
      expect(await userVersion(upgradedAgain), DatabaseHelper.databaseVersion);
  ...
  group('test seam and runtime profile', () {
    test('seam opens use production callbacks without singleton mutation',
      ...
      try {
        expect(await userVersion(seamDb), DatabaseHelper.databaseVersion);
        expect(
        ...
        expect(await userVersion(singletonDb), DatabaseHelper.databaseVersion);
      } finally {
        await seamDb.close();
